...
validation = open(r"D:\DNA synthesis\170523_454&illumina\%s\%s_analysis.txt" %(tp, tp), "r")
val_lines = validation.readlines()

real = open(r"D:\DNA synthesis\170523_454&illumina\%s\%s_real_variant.txt" %(tp, tp), "w")
real.write("Well" + "\t" + "Pos" + "\t" + "Ref" + "\t" + "454" + "\t" + "ill" + "\t" + "Freq" + "\t" + "(variant/normal)" + "\n")
real_count = 0

error = open(r"D:\DNA synthesis\170523_454&illumina\%s\%s_sequencing_error.txt" %(tp, tp), "w")
error.write("Well" + "\t" + "Pos" + "\t" + "Ref" + "\t" + "454" + "\t" + "ill" + "\t" + "Freq" + "\t" + "(variant/normal)" + "\n")
error_count = 0

etc = open(r"D:\DNA synthesis\170523_454&illumina\%s\%s_only_illumina.txt" %(tp, tp), "w")
...
for i in range(1, len(val_lines) - 1):
    if val_lines[i].split()[2] != val_lines[i].split()[3] and val_lines[i].split()[3] == val_lines[i].split()[4] and val_lines[i].split()[3] != "N":
        if float(val_lines[i].split()[5][:-1]) >= 90.0:
...
"""
real = open(r"D:\DNA synthesis\170523_454&illumina\%s\%s_real_variant.txt" %(tp, tp), "r")
error = open(r"D:\DNA synthesis\170523_454&illumina\%s\%s_sequencing_error.txt" %(tp, tp), "r")
etc = open(r"D:\DNA synthesis\170523_454&illumina\%s\%s_only_illumina.txt" %(tp, tp), "r")
